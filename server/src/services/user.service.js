...
const userService = {
  register: async (userData, next) => {
    ...
    const verificationCode = crypto.randomInt(100000, 999999); //six characters randomly generated
    const verificationCodeExpiry = new Date(Date.now() + 3600000); //1hr expiry
    //handle password encryption, for users
  ...
  authenticateUser: async (userId, next) => {
    //get userId from our just decoded token
  ...
  verifyUserAccount: async (data, next) => {
    ...
    //find our user, and get the verificationToken attached to this user/ and expiry saved to the user
    ...
    //check if verificationToken saved in the db is same as the one received from the form
  ...
  resendVerificationToken: async (userId, next) => {
    ...
    await user.save();
    process.nextTick(() => {
      mailService.sendVerification(user).catch(async (error) => {
        user.verificationToken = undefined;
        user.verificationTokenExpiry = undefined;
        await user.save();
